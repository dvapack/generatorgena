...
`outbox_messages` хранит команды, которые backend должен надёжно доставить в RabbitMQ.
Запись генерации и команда создаются в одной транзакции. После подтверждения RabbitMQ
команда получает `published_at`; при ошибке она остаётся в таблице для повторной попытки.
